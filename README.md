...
| 模块 | 说明 |
|------|------|
| 登录 / 用户管理 | 可创建多个平级登录账号；每个账号的数据和监听引擎相互独立 |
| 数据管理 | 商品新增 / 修改 / 删除 / 查询 / 备份导入导出 |
| 启用图片上传 | 给商品上传一张或多张图片，计算感知哈希；用户发来相似图片时自动识别并回复该商品 |
| 屏蔽关键字 | 可屏蔽用户文字消息；开启图片文字屏蔽后，也会 OCR 用户图片里的文字并命中屏蔽词 |
| Discord 账号 | 每个用户独立增删 Discord token |
...
1. **过滤**：先检查用户消息文字是否包含屏蔽关键字；如果开启
   `IMAGE_OCR_FILTER_ENABLED=1`，还会对用户消息里的所有图片做本地 OCR，
   任意图片里的文字命中屏蔽关键字就直接不回复。
2. **商品名**：提取用户消息和商品名中的字母/数字核心词，支持 `aj6 ↔ j6`
   变体、忽略大小写。系统会遍历全部启用商品并计算相似度，回复分数最高且超过阈值的商品链接。
3. **图片识别**：商品图片用 dHash + pHash（感知哈希）入库；用户消息里的每张图片
   ...
   > 需要更强能力时可接入 CLIP 等模型。
4. **未命中**：回退店铺信息（`CUSTOM_REPLY` + `SHOP_WEBSITE`），与原逻辑一致。
5. **回复方式**：`REPLY_MODE=reply` 时直接引用回复用户消息；`REPLY_MODE=thread`
   时在用户消息下创建 Discord 线程（Thread），默认线程名为 `Share links here`，
   再把回复内容发送到该线程中。
6. **消息处理记录**：`REPLY_LOG_ENABLED=1` 时，Discord 回复成功后会记录原消息 ID、
...
如果要识别中文图片文字，把系统配置里的 `IMAGE_OCR_LANGUAGES` 改成
`eng+chi_sim`，并额外安装中文语言包：
...
登录后进入「用户管理」，可以创建或切换其他平级账号。每个账号登录后可以独立配置：
